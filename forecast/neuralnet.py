...
def granger_causality(x1, x2, maxlag, addconst=True,
    ...
    with warnings.catch_warnings():
        ...
        gc = sm.tsa.stattools.grangercausalitytests(np.stack([x1, x2], axis=1),
                                                    maxlag=maxlag,
                                                    addconst=addconst,
                                                    verbose=False)
    # get all p_values
...
def granger_causality_lags_and_indices(yts, yt_target_idx, maxlag,
                                       minlag=1, addconst=True,
    ...
    for vi in range(n_var):
        ...
        pv, lag, vi_support = granger_causality(yts[:, yt_target_idx],
                                                yts[:, vi],
                                                maxlag=maxlag,
                                                addconst=addconst,
                                                p_thr=p_thr,
...
class NN_ICM(Forecaster):
    ...
    paper: From Paris to Berlin: Discovering Fashion Style Influences
        Around the World, https://arxiv.org/abs/2004.01316
    """

    def __init__(
            self, lags=[1, 2], nhidden=2,
            nepochs=500, batch_size=8, ninit=3, nval=4,
            nproc=1, cuda=False, seed=42,
            optimizer_opts=SN(optimizer='Adam', lr=1e-2, weight_decay=1e-8),
            scale_range=(-1, 1)):
    ...
    def _get_criterion(self):
        return losses.FCoherenceLoss(c_idx_list=self._idx_coherent,
    ...
    def reset(self, mlp_lags):
        """
        Resets the model params
        """
        l_nunints_list = self._get_nunits_list(mlp_lags)
        self.model = mlps.MLPGroup(l_nunints_list, mlp_lags,
    ...
    def _get_nunits_list(self, mlp_lags):
        ...
        for mi in range(len(mlp_lags)):
            if self.nhidden is None:
                ...
                l_nunints_list.append(
                    [len(mlp_lags[mi])] + [mi_nh] + hidden_nunits)
            else:
...
class AlphaWeight(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X, y=None):
        y_h = self.alpha * X[:, 0] + ((1 - self.alpha) * X[:, 1])
        return y_h
...
class NN_ICM_ENSA(NN_ICM):
    """
    Influence coherent model for forecasting style trends.
    This model combines both unit and style influences in an ensemble
    model that weights the forecasts with a learned alpha weight.
    Paper: Modeling Fashion Influence from Photos, https://arxiv.org/abs/2011.09663
    """

    def __init__(self, lags=[1, 2], nhidden=2,
                 nepochs=500, batch_size=8, ninit=3, nval=4,
                 nproc=1, cuda=False, seed=42,
                 optimizer_opts=SN(optimizer='Adam',
                                   lr=1e-2,
                                   weight_decay=1e-8),
                 scale_range=(-1, 1)
                 ):

        super(NN_ICM_ENSA, self).__init__(lags=lags, nhidden=nhidden,
                                          nepochs=nepochs,
                                          batch_size=batch_size, ninit=ninit,
                                          nval=nval,
                                          nproc=nproc, cuda=cuda, seed=seed,
                                          optimizer_opts=optimizer_opts,
                                          scale_range=scale_range)

        self.linear_reg_kvargs = {'param_grid':
    ...
    def reset(self, n_vars, n_units):
        self.icm_vars = []
        self.icm_untis = []
        self.n_vars = n_vars
        self.n_units = n_units
        for i in range(n_vars):
            self.icm_vars.append(
                NN_ICM(lags=self.lags,
        ...
        self.linear_reg = GridSearchCV(AlphaWeight(), **self.linear_reg_kvargs)

    def _forecast_val(self, total_steps, use_cuda=False):
        nsteps = self.nval
        start_step = -nsteps
        # unit models forecasts
        y_units = []
        for ui in range(self.n_units):
            y_units.append(self.icm_untis[ui]
                           .forecast(nsteps, use_cuda, start_step))

        # var models forecasts
        y_vars = []
        for vi in range(self.n_vars):
            y_vars.append(self.icm_vars[vi]
                          .forecast(nsteps, use_cuda, start_step))
        ...
        for ui in range(self.n_units):
            for vi in range(self.n_vars):
                y_2f.append(np.stack(
                            [y_units[ui][:, vi], y_vars[vi][:, ui]], axis=1))

        y_gf = np.concatenate(y_2f, axis=0)
        return y_gf

    def fit(self, yts, nepochs=None, batch_size=None,
            nworkers=None, ninit=None):
        umisc.set_random_seed(self.seed)
        # reset model
        n_steps = yts[0].shape[0]
        n_vars = yts[0].shape[1]
        n_units = len(yts)

        self.reset(n_vars, n_units)
        # prepare data
        yts_units = yts
        yts_vars = []
        for vi in range(n_vars):
            uib_traj = np.zeros((n_steps, n_units))
            for ui in range(n_units):
                uib_traj[:, ui] = yts_units[ui][:, vi]

            yts_vars.append(uib_traj.copy())

        # fit unit models
        for ui in tqdm(range(n_units), desc='Unit Models'):
            self.icm_untis[ui].fit(yts_units[ui],
                                   nepochs=nepochs,
                                   batch_size=batch_size,
                                   ninit=ninit)

        # fit var models
        for vi in tqdm(range(n_vars), desc='Var Models'):
            self.icm_vars[vi].fit(yts_vars[vi],
                                  nepochs=nepochs,
                                  batch_size=batch_size,
                                  ninit=ninit)

        # fit linear regressor
        # get forecasts on validation data
        y_val_f = self._forecast_val(n_steps, use_cuda=self.cuda)
        # get ground truth
        y_val_g_ls = []
        for ui in range(n_units):
            for vi in range(n_vars):
                y_val_g_ls.append(yts_units[ui][-self.nval:, vi])

        y_val_g = np.concatenate(y_val_g_ls, axis=0)
        # train a regressor
        self.linear_reg = self.linear_reg.fit(y_val_f, y_val_g)

        return True

    def forecast(self, nsteps, use_cuda=False):
        if len(self.icm_untis) == 0:
            raise ValueError('model not found, did you call fit() first?')

        # unit models forecasts
        y_units = []
        for ui in range(self.n_units):
            y_units.append(self.icm_untis[ui].forecast(nsteps, use_cuda))

        # var models forecasts
        y_vars = []
        for vi in range(self.n_vars):
            y_vars.append(self.icm_vars[vi].forecast(nsteps, use_cuda))

        # ensemble
        y_ensemble = copy.deepcopy(y_units)
        for ui in range(self.n_units):
            for vi in range(self.n_vars):
                y_ensemble[ui][:, vi] = self.linear_reg.predict(
                    np.stack((y_units[ui][:, vi], y_vars[vi][:, ui]), axis=1))
