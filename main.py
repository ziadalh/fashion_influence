...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Influence-based forecasting of style trends. '
        'project: https://www.cs.utexas.edu/~ziad/influence_from_photos.html')
    ...
    parser.add_argument('--influence_type', type=str,
                        choices=['ensemble', 'styles', 'units'],
                        default='ensemble',
                        help='type of influence to model among trends')
    ...
    # set variables
    dataset = traj_data['dataset']
    units_traj = traj_data['units_traj']
    units_list = traj_data['units_list']
    units_ids = traj_data['units_ids']
    num_units = traj_data['num_units']
    unit_type = traj_data['unit_type']
    num_vars = traj_data['num_vars']
    var_type = traj_data['var_type']
    ...
    logger.info('{}: trends loaded for {} units ({}) with {} styles each'
                .format(dataset, num_units, unit_type, num_vars))
    ...
    lags = [1, 2, seasonal_periods]
    if dataset == 'AmazonBrands':
        lags = [1, 2]

    if opt.influence_type == 'ensemble':
        logger.info('influence-based forecaster'
                    ' (models both {} and style influences)'.format(unit_type))
        finame = 'Influence-based'
        yts_train = []
        yts_test = []
        for ui in range(num_units):
            yts_train.append(units_traj[ui][:-ntest, :].copy())
            yts_test.append(units_traj[ui][-ntest:, :].copy())

        # setup influence model
        f_model = fnn.NN_ICM_ENSA(lags=lags, nproc=opt.nproc)
        succ = f_model.fit(yts_train)
        st_pred = f_model.forecast(ntest)
        # get forecaster error
        for ui in range(num_units):
            fmetrics.reset()
            fmetrics.update(st_pred[ui], yts_test[ui])
            models_error[finame][ui] = fmetrics.compute()

    elif opt.influence_type == 'units':
        logger.info('influence-based forecaster'
                    ' (models {} influences)'.format(unit_type))
        finame = 'Influence-based [{}]'.format(unit_type)
        # setup influence model
        infl_forecaster = partial(fnn.NN_ICM, lags=lags, nproc=opt.nproc)
    ...
    elif opt.influence_type == 'styles':
        logger.info('influence-based forecaster'
                    ' (models {} influences)'.format(var_type))
        finame = 'Influence-based [{}]'.format(var_type)
        # setup influence model
        infl_forecaster = partial(fnn.NN_ICM, lags=lags, nproc=opt.nproc)
        mvstyle_error = defaultdict(lambda: defaultdict(list))
        # for each unit
        for ui in tqdm(range(num_units)):
            ...
            # fit and forecast for style si across cities
            f_model = infl_forecaster()
            succ = f_model.fit(units_traj[ui][:-ntest, :])
            st_pred = f_model.forecast(ntest)
            nfails += 0 if succ else 1
            st_pred = np.clip(st_pred, 0, None)
            if np.isnan(st_pred).sum() > 0:
        ...
        raise ValueError('unknown influence_type [{}]'
                         .format(opt.influence_type))
    ...
    logger.info('\n{}: Forecast errors across {}-style trends\n{}'
                .format(dataset, unit_type, fb_error_tbl))
